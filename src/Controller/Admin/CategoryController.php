...
final class CategoryController extends AbstractController
{
    ...
    public function delete(Request $request, Category $category, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete' . $category->getId(), $request->getPayload()->getString('_token'))) {
            // Vérifiez si la catégorie contient des produits
            if ($category->getProducts()->count() > 0) {
                // Ajoutez un message flash pour indiquer qu'elle ne peut pas être supprimée
                $this->addFlash('error', 'Vous ne pouvez pas supprimer cette catégorie car elle contient des produits.');
                return $this->redirectToRoute('app_admin_category', [], Response::HTTP_SEE_OTHER);
            }

            // Supprimez la catégorie si elle ne contient pas de produits
